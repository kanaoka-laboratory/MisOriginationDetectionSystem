...
$page_max = ceil(($mysqli->query("select count(*) from SuspiciousAsnSet where suspicious_id<=66019 0<conflict_type and conflict_type<100")->fetch_assoc()["count(*)"])/$item_per_page);
if($page<0) $page=1;
...
<table id="hijack_list">
	...
	$query = "select * from SuspiciousAsnSet as t1 left join ".
			...
			"where t1.suspicious_id<=66019 and 0<conflict_type and conflict_type<100 order by prefix_set_count desc limit $limit_from, $item_per_page";
